...
export default function AllStaffRequest(){
    ...
    useEffect(()=>{
        function getPayments() {
            axios.get("http://localhost:8000/payment-server/payment/allpayment").then((res)=>{
  ...
  const deletePayments= async(id)=>{
    console.warn(id)
    let result = await fetch(`http://localhost:8000/payment-server/payment/deletepayment/${id}`,{
        method:"Delete"
    });
   ...
   const generatePDF = useReactToPrint({
    ...
    return(
        ...
        <div class="card bg-dark text-white" >
           <img src="img5.jpeg" class="card-img" alt="..." style={{height: '375px', filter: 'brightness(40%)'}}/>
             <div class="card-img-overlay">
              <br></br>
              <br></br>
                <h1 class="card-title" style={{fontSize: '90px'}}><b>ALL</b> <b style={{color: '#EAB333'}}>STATUS</b></h1>
                <br></br>
                <br></br>
                <br></br>
        ...
        <br></br>
        <form class="d-flex">
        <input class="form-control me-2 " type="text" onChange={(e) => setsearch(e.target.value)} placeholder="Search by course" aria-label="Search"/>
        </form>
        <br></br>
        <div>
        ...
        <table class="table table-striped" style={{ backgroundColor: '#D1CDCC'}} >
            ...
            {payments.filter((payments) => {
                return search.toString().toLowerCase() === '' ? payments: payments.studentName.toString().toLowerCase().includes(search);
                
            }).map((payments, index) => {
                return(
                <tr key={payments._id}>
                    <th scoop="row">{index+1}</th>
                    <td>{payments.studentName}</td> 
                    <td>{payments.CourseCode}</td>
                    <td>{payments.reqdate}</td>
                    <td>{payments.cnumber}</td>
                    <td>{payments.reqstatus}</td>
                    <td>
                        <a className="btn" style={{backgroundColor: '#EAB333',borderColor:'#EAB333'}} href={`/updatepayment/${payments._id}`}>
                            <i className="fas fa-edit"></i>&nbsp;Update
                        </a>

                        <h>    </h>
                        
                        <button className="btn btn-secondary" href="#" onClick={()=>deletePayments(payments._id)}>
